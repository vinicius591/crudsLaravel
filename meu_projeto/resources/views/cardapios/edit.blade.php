...
<main class="flex-1 p-6 md:p-12 flex items-center justify-center bg-gradient-to-br from-amber-50 to-orange-50 min-h-screen">
    <div class="w-full max-w-lg mx-auto">
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-sm text-amber-700/80 mt-1">Altere as informações do prato ou produto selecionado</p>
        </div>   

        <div class="bg-white py-8 px-6 shadow-xl rounded-3xl border-2 border-amber-100/50 sm:px-10">

            <form action="{{ route('cardapios.update', $cardapio->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label for="nome" class="block text-xs font-bold uppercase tracking-wider text-amber-800 mb-1">Nome do Item</label>
                    <input type="text" id="nome" name="nome" value="{{ old('nome', $cardapio->nome) }}" required placeholder="Ex: Risoto de Alho Poró"
                ...
                <div>
                    ...
                    <input type="number" id="preco" name="preco" step="0.01" value="{{ old('preco', $cardapio->preco) }}" required placeholder="0,00"
                           class="block w-full rounded-xl border-2 border-orange-100 bg-orange-50/30 px-4 py-3 text-gray-800 shadow-sm transition focus:border-orange-400 focus:bg-white focus:outline-none focus:ring-4 focus:ring-orange-100">
                    @error('preco')
                        <p class="mt-1 text-xs font-medium text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="disponivel" name="disponivel" required
                            class="block w-full rounded-xl border-2 border-orange-100 bg-orange-50/30 px-4 py-3 text-gray-800 shadow-sm transition focus:border-orange-400 focus:bg-white focus:outline-none focus:ring-4 focus:ring-orange-100">
                        <option value="1" {{ old('disponivel', $cardapio->disponivel) == '1' ? 'selected' : '' }}>Disponível imediatamente</option>
                        <option value="0" {{ old('disponivel', $cardapio->disponivel) == '0' ? 'selected' : '' }}>Indisponível / Fora de estoque</option>
                    </select>
                    @error('disponivel')
                        <p class="mt-1 text-xs font-medium text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center justify-end space-x-3 pt-4">
                    <a href="{{ route('cardapios.index') }}" class="w-1/2 sm:w-auto">
